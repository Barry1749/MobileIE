...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        real = ffted[..., 0]
        imag = ffted[..., 1]
        ffted_complex = torch.complex(real, imag)               # [B, C_out, H, W]

        output = torch.fft.ifft2(ffted_complex, dim=(-2, -1), norm="ortho").real
...
class FFC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.in_cl = in_channels - in_cg
        self.out_cg = int(out_channels * ratio_gout)
        self.out_cl = out_channels - out_cg
        #groups_g = 1 if groups == 1 else int(groups * ratio_gout)
        #groups_l = 1 if groups == 1 else groups - groups_g

        self.ratio_gin = ratio_gin
        # self.ratio_gin = nn.Parameter(torch.tensor(ratio_gin))
        self.ratio_gout = ratio_gout
        # self.ratio_gout = nn.Parameter(torch.tensor(ratio_gout))
    ...
    def forward(self, x):
        ...
        x_l = x[:, :self.in_cl, :, :]                    # [B, Cl, H, W]
        x_g = x[:, self.in_cl:, :, :] if self.in_cg > 0 else 0  # [B, Cg, H, W] or 0
        out_xl, out_xg = 0, 0
