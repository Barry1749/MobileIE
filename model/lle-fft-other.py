...
class MobileIELLENet(nn.Module):
    def __init__(self, channels, rep_scale=4):
        ...
        # self._freeze_head_body_att_tail()

        self.locv3_param_head = LocV3ParamHead()
        self.locv3_module = LocV3Module()

         

    def _freeze_head_body_att_tail(self):
        # choose what to freeze
        modules_to_freeze = [self.head, self.body, self.att, self.tail]
    ...
    def forward(self, x):
        ...
        # return self.tail(x2)
        x3 = self.tail(x2)
        locv3_params = self.locv3_param_head(x, x3)
